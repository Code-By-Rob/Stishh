...
postArticleButton.addEventListener('click', createArticleFunction);
function createArticleFunction() {
...
function postArticleToDatabase(link) {
    ...
    .then(data => {
        ...
        if (data.message === 'success') {
            successfullyAddedToDatabase(data.articleID);
        } else if (data.message === 'failure') {
            unsuccessfullyPostedToDatabase()
        }
    })
    .catch(err => {
        console.log(err);
...
function successfullyAddedToDatabase(articleID) {
    // when the message sent back from the server is 'success'
    postArticleButton.textContent = 'Go To Article!';
    postArticleButton.removeEventListener('click', createArticleFunction);
    postArticleButton.addEventListener('click', () => {
        window.location.replace(`https://www.stishh.com/article/${articleID}`);
    })
}
function unsuccessfullyPostedToDatabase() {
    // when the message sent back from the server is 'failure'
    console.log('There was a failure posting the article');
}
...
try {
    createDraftBtn.addEventListener('click', createDraftFunction);
} catch (err) {
    console.log(err);
}
function createDraftFunction() {
